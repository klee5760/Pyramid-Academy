...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while(true)
        {
            ...
            System.out.println("PLAYER MAKE GUESS");
            result = askForGuess(userPlayer, computer);

            if (userPlayer.playerGrid.hasLost())
            {
                System.out.println("Computer sunk your last ship! You lost!");
                break;
            }
            else if (computer.playerGrid.hasLost())
            {
                System.out.println("You sunk the opponent's last ship! You won!");
                break;
            }
...
private static String askForGuess(Player p,Player opp) {
    ...
    while (true) {
        ...
        System.out.print("Type in colume(1-10): ");
        col = reader.nextInt();
        oldCol = col;
            ...
            if (dir == 0) {
                ...
                for (int i = row; i < row + length; i++) {
                    if (p.playerGrid.hasShip(i, col)) {
                        System.out.println("THERE IS ALREADY A SHIP AT THE LOCATION");
                        return true;
                    }
                }
            }
